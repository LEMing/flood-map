// GLSL shader source for the raymarched volumetric cloud dome, split out of
// CloudDome.ts so the factory module stays within the max-lines budget. The
// strings are GPU shader source moved verbatim — do NOT reformat or re-indent.

export const CLOUD_VERT = `// Dome vertex shader. The sphere is recentered on the camera every frame, so
...
export const CLOUD_FRAG = `precision highp float;
